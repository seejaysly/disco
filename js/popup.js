    ...
    citable.directive('tagsDirective', ['$compile', function ($compile) {
        return {
            ...
            link: function ($scope, $element, $attrs) {
                ...
                $scope.tagsInput = new Tags.TagInput('Tags', '', (items) => {
        ...
        $scope.getDocument = function (param) {
            ...
            bgPage.getDocument(param, $scope.data.defaultDoc.id, true, function (response) {
                if (response != null) {
                    ...
                    msgService.queue(status + "Developpement in progress", 'error');
                }
    ...
    citable.controller('DocsController', ['$scope', '$http', '$timeout', 'gdocs', 'sharedProps', 'keyboardManager', 'msgService', function ($scope, $http, $timeout, gdocs, sharedProps, keyboardManager, msgService) {
        ...
        function successCallbackWithFsCaching(resp, status, headers, config, statusText) {
            ...
            if (!resp.items.length) {
                ...
                msgService.queue('Welcome to Disco!', 'normal', 2000);
        ...
        $scope.fetchFolder = function (retry, opt_config) {
            ...
            if (gdocs.accessToken) {
                ...
                //query: is a folder, properties contains Disco=true, is visible to PUBLIC, and isn't in the trash.
        ...
        $scope.amendDoc = function (retry, destination, callback) {
            ...
            if ((destination === null || destination.id == '')) { // && $scope.data.newDoc //Manual validation... icky.
                ...
                amendDocHandler(retry, destination.id, callback, destination); //Call the handler passing in the formatted values.
            }
        };

        amendDocHandler = async function (retry, docId, callback, destinat) {
            ...
            if (gdocs.accessToken) {
                ...
                var tempData = await fetch(bgPage.SPREAD_SCOPE + '/spreadsheets/' + docId + "?includeGridData=false", {
                ...
                var worksheetId = tempData.sheets[0].properties.title;
                var url = bgPage.SPREAD_SCOPE + '/spreadsheets/' + docId + '/values/' + worksheetId + ':append?valueInputOption=RAW&responseValueRenderOption=UNFORMATTED_VALUE&insertDataOption=INSERT_ROWS&responseDateTimeRenderOption=SERIAL_NUMBER&includeValuesInResponse=true&alt=json';
                ...
                var checkResponse = function (response) {
                    var comment = document.querySelector('#comment-id-24').value
                    $scope.data.citation.Comment = comment
                    var citation = $scope.data.citation;
                    
                    var doc = response.data;
                    editLink = `https://sheets.googleapis.com/v4/spreadsheets/${doc.updates.spreadsheetId}/values/${doc.tableRange}`
                    ...
                    for (var key in citation) {
                        x++
                        var foundColumn = (doc.updates.updatedData.values[0][x] == "" || doc.updates.updatedData.values[0][x].indexOf(" unavailable)") != -1) ? null : doc.updates.updatedData.values[0][x]
                        if (foundColumn === null) {
        ...
        var constructCitation = function () {
            ...
            Object.entries($scope.data.citation).map((entry, index, array) => {
                temp.values[0].push(entry[1] == "" ? `(${entry[0]} unavailable)` : entry[1])
            })
            return temp
        };
